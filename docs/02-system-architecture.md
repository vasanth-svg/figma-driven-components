...
The live-preview requirement is feasible when implemented as controlled worker-owned previews. The website should show structured events, an Expo Snack live preview, recordings, logs, diffs, and approvals, but it should not expose an interactive terminal or IDE in v1.
...
5. Worker publishes a Snack-compatible preview entry and refreshes the Expo Snack embed URL for the changed UI.
